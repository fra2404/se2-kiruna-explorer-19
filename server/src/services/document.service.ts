...
// Update document
export const updatingDocument = async (id: string, updateData: Partial<IDocument>): Promise<IDocumentResponse | null> => {
    ...
    if (updateData.coordinates) {
        const existingCoordinate = await Coordinate.findById(updateData.coordinates);
        if (!existingCoordinate) {
            throw new PositionError();
        }
    }
            ...
            for (const connection of updatedDocument.connections) {
                ...
                if (existingDocument) {
                    existingDocument.connections = existingDocument.connections?.filter(
                        (conn) => conn.document.toString() !== updatedDocument.id.toString()
                    );
                    await existingDocument.save();
                }
            }
    ...
    if (updatedDocument.coordinates) {
        coordinates = await getCoordinateById(updatedDocument.coordinates.toString());
    }
